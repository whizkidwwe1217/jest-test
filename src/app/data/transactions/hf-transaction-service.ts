...
import { BaseTransactionService } from "./base-transaction";
import { TransactionType } from "./transaction-type";
import * as _ from "lodash";

@Injectable()
export class HfTransactionService<
	T extends Transaction<T, K>,
	K,
	S extends State<T>
> extends BaseTransactionService<T, K, S> {
	...
	protected _states: Map<K, S> = new Map();
	 ...
	 * @inheritdoc
	 */
	public add(transaction: T, recordRef?: T): void {
		const states = this._isPending ? this._pendingStates : this._states;
		this.verifyAddedTransaction(states, transaction, recordRef);
		this.addTransaction(transaction, states, recordRef);
	}

	protected addTransaction(transaction: T, states: Map<K, S>, recordRef?: T) {
		this.updateState(states, transaction, recordRef);

		const transactions = this._isPending
	 ...
	 * @inheritdoc
	 */
	public getTransactionLog(id?: K): T[] {
		if (id) {
	...
	public getAggregatedChanges(mergeChanges: boolean): T[] {
		const result: T[] = [];
		this._states.forEach((state: S, key: K) => {
			const value = mergeChanges
				? this.mergeValues(state.recordRef, state.value)
				: state.value;
	...
	public commit(data: T[]): void {
		this._states.forEach((s: S) => {
			const index = data.findIndex(
				i => JSON.stringify(i) === JSON.stringify(s.recordRef)
			);
			switch (s.type) {
	...
	public undo(): void {
		...
		this._transactions.splice(
			this._transactions.length - lastActions.length
		);
	 ...
	 */
	protected verifyAddedTransaction(
		states: Map<K, S>,
		transaction: T,
		recordRef?: any
	): void {
		...
		switch (transaction.type) {
			case TransactionType.ADD:
				if (state) {
					...
					throw new Error(
						`Cannot add this transaction. Transaction with id: ${
							transaction.id
			...
			case TransactionType.UPDATE:
				...
				if (!state && !recordRef && !this._isPending) {
					...
					throw new Error(
						`Cannot add this transaction. This is first transaction of type ${
							transaction.type
						} ` +
							`for id ${
								transaction.id
							}. For first transaction of this type recordRef is mandatory.`
					);
				}
				break;
		}
	 ...
	 */
	protected cleanState(id: K, states: Map<K, S>): void {
		...
		if (state && state.value && state.recordRef) {
			...
			if (_.isObject(state.recordRef)) {
				for (const key of Object.keys(state.value)) {
					if (
						JSON.stringify(state.recordRef[key]) ===
