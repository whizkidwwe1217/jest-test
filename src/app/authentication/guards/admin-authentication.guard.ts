...
import { SET_INFO } from "../../data/actions/app-info.action";
...
@Injectable()
export class AdminAuthenticationGuard implements CanActivate, CanActivateChild, CanLoad {
	appInfo: AppInfo;
	...
	checkLoggedAccount(url: string): Observable<boolean> {
		...
		return this.http.getValue<AppInfo>(URL_APP_INFO).pipe(
			switchMap((info: AppInfo) => {
				...
				if (!account) {
					account = JSON.parse(localStorage.getItem(LS_CATALOG_AUTH_ACCOUNT));
				}
				if (account) {
					if (info.isAdmin && account.username && account.password) {
						return of(true);
					}
				}
