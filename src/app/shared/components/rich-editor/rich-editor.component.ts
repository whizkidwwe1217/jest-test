...
import { RichEditorConfig, richEditorConfig } from "./config";
...
export class RichEditorComponent
	...
	@Input() id = "";
	@Input() config: RichEditorConfig = richEditorConfig;

	@Output() html;

	@ViewChild("editor", { static: true }) textArea: any;
	@ViewChild("editorWrapper", { static: true }) editorWrapper: any;
	@ViewChild("editorToolbar", { static: true }) editorToolbar: RichEditorToolbarComponent;
	...
	constructor(
		...
		private editorService: RichEditorService,
		@Inject(DOCUMENT) private _document: any
	) {}

	ngOnInit() {
		this.editorToolbar.id = this.id;
		this.editorToolbar.fonts = this.config.fonts
			? this.config.fonts
			: richEditorConfig.fonts;
	...
	ngAfterContentInit() {
		if (this.config.defaultFontName) {
			this.editorToolbar.defaultFontId = this.config.defaultFontName
				? this.editorToolbar.fonts.findIndex(x => {
	...
	onTextAreaBlur(event: FocusEvent) {
		...
		if (
			...
			(event.relatedTarget as HTMLElement).parentElement.className !==
				"rich-editor-toolbar-set"
		) {
			this.blur.emit("blur");
		}
	}
	...
	toggleEditorMode(bToSource: boolean) {
		...
		if (bToSource) {
			...
			oCode.setAttribute(
				"style",
				"white-space: pre-wrap; word-break: keep-all; margin: 0; outline: none; background-color: #fff5b9;"
			);
