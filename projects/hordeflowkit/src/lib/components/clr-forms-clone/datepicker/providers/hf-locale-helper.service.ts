 ...
 */
@Injectable()
export class HfLocaleHelperService {
  constructor(@Inject(LOCALE_ID) public locale: string) {
    this.initializeLocaleData();
  }

  private _firstDayOfWeek: number = 0;
  private _localeDays: ReadonlyArray<HfDayOfWeek>;
  ...
  private initializeLocaleDays(): void {
    ...
    const tempArr = [];
    const tempWideArr: string[] = getLocaleDayNames(
      this.locale,
  ...
  private initializeLocaleMonthsWide(): void {
    this._localeMonthsWide = getLocaleMonthNames(
      this.locale,
      FormStyle.Standalone,
      TranslationWidth.Wide
    ).slice();
