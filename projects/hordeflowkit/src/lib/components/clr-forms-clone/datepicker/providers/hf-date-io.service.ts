...
import { HfLocaleHelperService } from "./hf-locale-helper.service";

@Injectable()
export class HfDateIOService {
  public disabledDates: HfDateRange = {
    ...
    minDate: new HfDayModel(0, 0, 1),
    maxDate: new HfDayModel(9999, 11, 31)
  };
  ...
  constructor(private _localeHelperService: HfLocaleHelperService) {
  ...
  public setMinDate(date: string): void {
    ...
    if (!date) {
      ...
      this.disabledDates.minDate = new HfDayModel(0, 0, 1);
    } else {
      const [year, month, day] = date.split("-").map(n => parseInt(n, 10));
      this.disabledDates.minDate = new HfDayModel(year, month - 1, day);
    }
  }
  ...
  public setMaxDate(date: string): void {
    ...
    if (!date) {
      ...
      this.disabledDates.maxDate = new HfDayModel(9999, 11, 31);
    } else {
      const [year, month, day] = date.split("-").map(n => parseInt(n, 10));
      this.disabledDates.maxDate = new HfDayModel(year, month - 1, day);
    }
  }
