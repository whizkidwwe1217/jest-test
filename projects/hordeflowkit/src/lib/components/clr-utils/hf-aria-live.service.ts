...
import { uniqueHfIdFactory } from "./hf-id-generator.service";

export enum HfAriaLivePoliteness {
  off = "off",
  polite = "polite",
  assertive = "assertive"
}
 ...
 */
@Injectable()
export class HfAriaLiveService implements OnDestroy {
  ...
  private _id: string = `clr-aria-live-element-${uniqueHfIdFactory()}`;
  /**
  ...
  public announce(
    ...
    politeness: HfAriaLivePoliteness = HfAriaLivePoliteness.polite
  ): void {
    if (politeness === HfAriaLivePoliteness.off) {
      return;
    }
    ...
    message =
      typeof message !== "string" && isPlatformBrowser(this.platformId)
        ? message.textContent
        : message;
  ...
  private createContainer(): HTMLElement {
    ...
    ariaLiveElement.setAttribute("aria-live", HfAriaLivePoliteness.polite);
