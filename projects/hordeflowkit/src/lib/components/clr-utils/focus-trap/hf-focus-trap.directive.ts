...
import { HfFocusTrapTracker } from "./hf-focus-trap-tracker.service";

interface HfFocusTrapConfig {
  strict: boolean;
}

@Directive({ selector: "[hfFocusTrap]" })
export class HfFocusTrapDirective implements AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private focusTrapsTracker: HfFocusTrapTracker,
  ...
  private _config: HfFocusTrapConfig = {
    strict: true
  };
  @Input("hfFocusTrap")
  set config(config: HfFocusTrapConfig) {
  ...
  onFocusIn(event: any) {
    ...
    if (this._config.strict && event.target && !nativeElement.contains(event.target)) {
      ...
      event.target === this.bottomReboundEl &&
      nativeElement.contains(this.previousTrappedActiveElement)
    ) {
