...
@Component({
  ...
  animations: [
    ...
    trigger("move", [
      state("currentPosition", style({ transform: "translateY({{absolute}}px)" }), {
        params: { absolute: "0" }
      }),
      state("moveUp", style({ transform: "translateY({{absolute}}px)" }), {
        params: { absolute: "0" }
      }),
      state("moveDown", style({ transform: "translateY({{absolute}}px)" }), {
        params: { absolute: "0" }
      }),
    ...
    trigger("fade", [
      state("fadeOut", style({ transform: "translateY({{absolute}}px)", opacity: 0 }), {
        params: { absolute: "0" }
      }),
...
export class ClrNotification implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.timeout > 0) {
      if (this.progressbar) {
        ...
        this.interval = zonedInterval(
          this.timeout / (this.timeout / this.step),
          this.ngZone
  ...
  public close(): void {
    ...
    this.state = {
      value: "fadeOut",
      params: { absolute: this._translate }
    };
