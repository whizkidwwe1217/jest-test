...
export class ClrNotificationService {
	...
	openNotification(content: any, options: ClrNotificationOptions = {}): ClrNotificationRef {
		...
		const notificationRef: ClrNotificationRef = new ClrNotificationRef(
			notificationCmptRef,
			contentRef
		);

		this._applyWindowOptions(notificationCmptRef.instance, options);
		notificationCmptRef.instance.closed.subscribe(
			this._afterClose.bind(this, notificationCmptRef.instance)
		);
	...
	private _attachWindowComponent(
		containerEl: any,
		contentRef: ClrContentRef
	): ComponentRef<ClrNotification> {
		const containerFactory = this._componentFactoryResolver.resolveComponentFactory(
			ClrNotification
		);
